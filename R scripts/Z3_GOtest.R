...
interestingGenes = fread(inputIntGenes, data.table=FALSE, fill=TRUE, header=FALSE)

#table with all genes, marked as interesting or not
geneList = (factor(as.integer(universeGenes[,1] %in% interestingGenes[,1])))
...
pdf(paste(outputPrefix, "_GO_graphs.pdf", sep= ""))
...
write.table(resMF, paste(outputPrefix, "_GO_resultsMF.txt", sep=""), quote=FALSE, sep='\t')
write.table(resBP, paste(outputPrefix, "_GO_resultsBP.txt", sep=""), quote=FALSE, sep='\t')
write.table(resCC, paste(outputPrefix, "_GO_resultsCC.txt", sep=""), quote=FALSE, sep='\t')
q()
